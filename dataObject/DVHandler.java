...
public class DVHandler implements Runnable {
    ...
    private Hashtable<Integer,String> tableRoutageDVPourTransfert(){
        ...
        while (it.hasNext()) {
          ...
          logger.info("DVHandler-:" + myRouteur.getNomRouteur()+ ":JE SUIS ROUTEUR" + tempRouteur.getNomRouteur() +" VERS PORT  "+ tempKey);
          //On ajoute l'élément à la table pour l'export
          //tablePourExport.put(tempKey, tempRouteur.getNomRouteur());
         
          String nomRouteurCherche = null;
          
          for (Routeur value : myRouteur.getListeRouteurs().values()) 
          {
        	  if (value.getPort()==tempKey)
        	  {
        		  nomRouteurCherche = value.getNomRouteur();
        	  }
        	  
          }
          tablePourExport.put(tempKey,nomRouteurCherche );
        }
